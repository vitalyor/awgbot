...
def _get_state_profile(st: dict, tid: int, name: str) -> tuple[dict | None, dict | None]:
...
def sync_absent_apply_one(tid: int, name: str) -> tuple[bool, str]:
    ...
    if not urec:
        _log_apply("sync_absent_apply_one", tid=tid, name=name, ok=False, reason="user_not_in_state")
        return False, "user_not_in_state"
    if not pr:
        _log_apply("sync_absent_apply_one", tid=tid, name=name, ok=False, reason="profile_not_in_state")
        return False, "profile_not_in_state"
    if pr.get("suspended"):
        _log_apply("sync_absent_apply_one", tid=tid, name=name, ok=False, reason="profile_suspended")
        return False, "profile_suspended"

    try:
        if XR.find_user(tid, name):
            _log_apply("sync_absent_apply_one", tid=tid, name=name, ok=False, reason="already_present")
    ...
    try:
        ...
        save_state(st)
        _log_apply("sync_absent_apply_one", tid=tid, name=name, ok=True, reason="ok", uuid=pr.get("uuid"))
        return True, "ok"
    except Exception as e:
        _log_apply("sync_absent_apply_one", tid=tid, name=name, ok=False, reason="xray_add_fail", error=str(e))
...
def sync_absent_apply_all() -> dict:
    ...
    for it in items:
        ...
        if ok:
            ...
            if reason in ("user_not_in_state", "profile_not_in_state", "profile_suspended", "already_present"):
                skipped += 1
            else:
                errors += 1
...
def sync_extra_apply_one(tid: int, name: str) -> tuple[bool, str]:
    ...
    if not target:
        _log_apply("sync_extra_apply_one", tid=tid, name=name, ok=False, reason="not_found_in_xray")
        return False, "not_found_in_xray"

    try:
        ok = XR.remove_user_by_name(tid, name)
        if not ok:
            _log_apply("sync_extra_apply_one", tid=tid, name=name, ok=False, reason="xray_remove_fail")
            return False, "xray_remove_fail"
        _log_apply("sync_extra_apply_one", tid=tid, name=name, ok=True, reason="ok", uuid=target.get("uuid"))
        return True, "ok"
    except Exception as e:
        _log_apply("sync_extra_apply_one", tid=tid, name=name, ok=False, reason="xray_remove_fail", error=str(e))
        return False, "xray_remove_fail"
